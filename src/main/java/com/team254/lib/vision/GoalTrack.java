...
public class GoalTrack {
    TreeMap<Double, RigidTransform2> mObservedPositions = new TreeMap<>();
    RigidTransform2 mSmoothedPosition = null;
    int mId;
     ...
     */
    public static synchronized GoalTrack makeNewTrack(double timestamp, RigidTransform2 first_observation, int id) {
     ...
     */
    public synchronized boolean tryUpdate(double timestamp, RigidTransform2 new_observation) {
        if (!isAlive()) {
            return false;
        }
    ...
    synchronized void smooth() {
        if (isAlive()) {
            ...
            int num_samples = 0;
            for (Map.Entry<Double, RigidTransform2> entry : mObservedPositions.entrySet()) {
            ...
            if (num_samples == 0) {
                ...
                mSmoothedPosition = new RigidTransform2(new Vector2(x,y), new Rotation2(c, s, true));
            }
        }
    }

    public synchronized RigidTransform2 getSmoothedPosition() {
        return mSmoothedPosition;
    }

    public synchronized RigidTransform2 getLatestPosition() {
